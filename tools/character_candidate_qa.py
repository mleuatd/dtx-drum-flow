...
def centroid(im):
    ...
    for y in range(bbox[1],bbox[3]):
        for x in range(bbox[0],bbox[2]):
            w=px[x,y]
            if w: sx+=x*w;sy+=y*w;sw+=w
    return None if not sw else [sx/sw,sy/sw]

def diff_metrics(a,b):
    if a.size!=b.size:return {"comparable":False}
    ...
    return {"comparable":True,"changedPixelRatio":changed/total,"meanChannelDiff":stat.mean,
            "bboxA":alpha_bbox(a.convert("RGBA")),"bboxB":alpha_bbox(b.convert("RGBA")),
            "centroidA":centroid(a.convert("RGBA")),"centroidB":centroid(b.convert("RGBA"))}
...
try:
    ...
    res["status"]="FAIL";res["errors"].append("not readable image: "+str(e))
else:
    res.update({"format":im.format,"mode":im.mode,"size":list(im.size),"alphaBBox":alpha_bbox(im.convert("RGBA"))})
    if im.format!="PNG":res["errors"].append("format is not PNG")
    if im.size!=(W,H):res["errors"].append(f"dimensions {im.size} != {(W,H)}")
    if im.mode!="RGBA":res["warnings"].append("source mode is not RGBA")
    rgba=im.convert("RGBA"); a=rgba.getchannel("A"); ext=a.getextrema()
    if ext[0]==255:res["errors"].append("no transparent pixels")
    if ext[1]==0:res["errors"].append("empty alpha image")
    bbox=a.getbbox()
    if bbox:
        bw,bh=bbox[2]-bbox[0],bbox[3]-bbox[1]
        if bw>W*.96 and bh>H*.96:res["warnings"].append("alpha bbox nearly fills whole canvas; inspect opaque background/extreme crop")
        if bw<W*.08 or bh<H*.08:res["warnings"].append("very small visible bbox; possible empty/cropped candidate")
    if args.expected_name and p.name!=args.expected_name:res["warnings"].append("filename differs from expected: "+args.expected_name)
    if args.neutral:
