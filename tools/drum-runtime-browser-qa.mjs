...
const report={...result,errors,status:(result.state==="running"&&result.buffers>=10&&result.rows.every(x=>x.bufferCount>0)&&result.rows.some(x=>x.activeSoon>0)&&result.comboRows.some(x=>x.activeSoon>x.before)&&result.denseActive>=5&&errors.length===0)?"PASS":"FAIL"};
