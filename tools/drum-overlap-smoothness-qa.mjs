...
let peak=0,clip=0,maxJump=0,jumpAt=0;for(let i=0;i<N;i++){peak=Math.max(peak,Math.abs(x[i]));if(Math.abs(x[i])>=.999)clip++;if(i){let j=Math.abs(x[i]-x[i-1]);if(j>maxJump){maxJump=j;jumpAt=i/SR}}}
...
const report={sampleRate:SR,duration:DUR,events:seq.length,peak,clippedSamples:clip,maxAdjacentJump:maxJump,maxJumpAtSec:jumpAt,onsetWindows,status:clip===0&&maxJump<1.25?"PASS":"REVIEW"};
