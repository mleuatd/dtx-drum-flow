...
summary={"schemaVersion":3,"issueId":"MOTION-004","actionKey":"RC+SN:R/L",
...
for phase in ("hit","rebound"):
    ...
    sn_mask=(dil(sd,21)&geom) | (neutral_left & dil(sd|diff(I["neutral"],sn),13))
    ...
    checks={
      ...
      "hairCoreLocked":guard(changed,HAIR_CORE)==0,
      "pelvisLocked":guard(changed,PELVIS)==0,
      "stoolLocked":guard(changed,STOOL)==0,
      "lowerBodyLocked":int(np.count_nonzero(changed[620:,:]))==0,
      "rcDonorExact":rpres>=0.995,
      "snDonorExact":spres>=0.985,
      "bothArmsVisible":int(np.count_nonzero(rc_mask&changed))>500 and int(np.count_nonzero(sn_mask&changed))>500,
    }
    cp=OUT/f"rc_sn_{phase}_candidate_v3.png"; cand.save(cp)
    Image.alpha_composite(I["drum"],cand).save(OUT/f"rc_sn_{phase}_candidate_v3_fixed_drum.png")
    ...
    (OUT/f"rc_sn_{phase}_candidate_v3_qa.json").write_text(json.dumps(rec,indent=2)+"\n")
    summary["phases"].append(rec)

# Visual transition evidence.
frames=[I["neutral"],Image.open(OUT/"rc_sn_hit_candidate_v3.png").convert("RGBA"),Image.open(OUT/"rc_sn_rebound_candidate_v3.png").convert("RGBA"),I["neutral"]]
...
strip.save(OUT/"rc_sn_v3_transition.jpg",quality=95)
summary["pairMachinePass"]=all(x["machinePass"] for x in summary["phases"])
(OUT/"M004_V3_SUMMARY.json").write_text(json.dumps(summary,indent=2)+"\n")
