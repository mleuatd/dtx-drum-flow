...
summary_path=WORK/"METHOD2_CANDIDATE_SUMMARY.json"
...
for rec in summary.get("targets",[]):
    ...
    if not rec.get("pass"):
        t["brushupStatus"]="AUTO_REPAIR_METHOD2_FAIL"
    ...
    if e:
        ...
        e["motionRepairMethod"]="cause-specific method2: active RF leg corridor + narrow contact corridor"
...
for d in backlog.get("defects",[]):
    ...
    if p:
        d["autoRepairPromotion"]={"promoted":p,"method":"cause-specific method2: active RF leg corridor + narrow contact corridor","candidateSummary":str(summary_path.relative_to(ROOT))}
        ...
        d["status"]="AUTO_REPAIR_METHOD3_REQUIRED"
        d["repairSpec"]["state"]="IN_FIX"
        d["updatedAt"]="2026-09-20T05:45:00+09:00"
...
(WORK/"METHOD2_PROMOTION_RESULT.json").write_text(json.dumps(record,ensure_ascii=False,indent=2)+"\n")
