...
prev=ROOT/"character-assets/edit-workspaces/motion-repair-20260920/method2/METHOD2_PROMOTION_RESULT.json"
...
out={"schemaVersion":3,"method":"instrument-side forearm/stick corridor with neutral core lock","targets":[]}
for f in fails:
    ...
    for part,limb in comps:
        ...
        if limb in ("L","R") and target:
            ...
            # Allow a corridor to the instrument, but core lock below removes torso contamination.
            width=165 if phase=="hit" else 190
            d.line([sh,target],fill=255,width=width)
            rr=90 if phase=="hit" else 120
    ...
    exact=np.all(Q==S,axis=2); changed=int(nd.sum()); ratio=changed/(W*H)
    preservation=[]
    for part,limb in comps:
        target=cp(part)
    ...
    checks={
      "changedVisible":changed>=250,"ratioWithinLimit":ratio <= (.10 if len(comps)==1 else .16),
      "outsideMaskZero":int(np.count_nonzero(nd&~M))==0,
      "headLocked":guard(nd,HEAD)==0,"torsoCoreLocked":guard(nd,TORSO_CORE)==0,"stoolLocked":guard(nd,STOOL)==0,
    ...
    rec={"id":tid,"actionKey":key,"phase":phase,"source":t["formalGitHubPath"],"sourceSha256":hashlib.sha256(srcp.read_bytes()).hexdigest(),
      ...
      "headGuardChangedPixels":guard(nd,HEAD),"torsoCoreGuardChangedPixels":guard(nd,TORSO_CORE),
...
(WORK/"METHOD3_CANDIDATE_SUMMARY.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
