...
def main():
    ...
    q=load_json(a.pose); base=Image.open(a.drum).convert("RGBA")
    if a.candidate:
        base=Image.alpha_composite(base,Image.open(a.candidate).convert("RGBA"))
    draw=ImageDraw.Draw(base)
    for name,pt in q.get("joints",{}).items():
        if isinstance(pt,list) and len(pt)==2:
            draw.ellipse((pt[0]-4,pt[1]-4,pt[0]+4,pt[1]+4),outline=(0,0,0,255),width=2)
    ...
    for x,y in pairs:
        if x in q["joints"] and y in q["joints"]:
            draw.line([q["joints"][x],q["joints"][y]],fill=(0,0,0,255),width=2)
