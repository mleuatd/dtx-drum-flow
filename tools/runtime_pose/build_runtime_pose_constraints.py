...
def main():
    ...
    out={"schemaVersion":1,"authority":"tool","actionKey":a.action_key,"phase":a.phase,
         "instrument":part,"limb":limb,"targetHeadCount":ret["targetHeadCount"],
         ...
         "canvas":contacts["canvas"],"contactPoint":target,
         ...
         "tolerancesPx":{"stickTip":8,"stool":12,"hip":18,"transitionJoint":120},
         "confidence":state.get("confidence",0)}
    if limb in ("L","R"):
        side=limb.lower()
        out["stickTip"]=state["props"].get("stick_tip_"+side)
        if a.phase=="hit" and target:
            ...
            out["reboundRule"]="preserve source rebound tip away from contact; validate continuity, do not force contact"
    elif target and limb in ("LF","RF") and a.phase=="hit":
        ...
        out["pedalTarget"]=target; out["legReach"]=solved
