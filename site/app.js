...
import {initCharacterPrototype,updateCharacterPrototype} from "./character-prototype.js?v=20260917-1-16-runtime";
...
function setChart(c){pausePlayback(false);chart=c;time=0;nextNote=0;$("timeline").max=c.duration;$("duration").textContent=fmt(c.duration);$("chartName").textContent=c.name;$("chartMeta").textContent=`${Math.round(c.bpm)} BPM · ${c.duration.toFixed(1)}秒 · ${c.notes.length.toLocaleString()}ノーツ`;const name=c.name||"";let stage="LUNA / DRUM CHART",badge="LUNA FINAL";if(name.includes("叶えたい")){stage="叶えたい、ことばかり / DRUM CHART";badge="KANAETAI FINAL"}else if(name.includes("一体いつから")){stage="一体いつから / DRUM CHART";badge="ITTAI FINAL"}if($("stageTitle"))$("stageTitle").textContent=stage;if($("chartBadge"))$("chartBadge").textContent=badge;draw();updateTime()}
