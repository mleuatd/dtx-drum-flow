...
const ASSET_VERSION="20260918-neutral-home-644ccf0a-r1";
...
function updateDevHud(extra=""){
  ...
  const lines=[
    "BUILD 20260918-drum-compact-v2-r1ntactfix-r1",
...
function handForNote(note){if(note?.limb)return note.limb;if(note?.animation?.limb)return note.animation.limb;if(note?.animation?.hand)return note.animation.hand;if(note?.animation?.foot)return note.animation.foot;if(note?.part==="SN")return "L";if(note?.part==="BD"||note?.part==="LB")return "RF";if(note?.part==="LP")return "LF";return "R"}
